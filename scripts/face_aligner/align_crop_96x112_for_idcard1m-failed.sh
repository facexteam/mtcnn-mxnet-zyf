...
if [ $splits -lt 2 ]; then
    nohup python batch_mtcnn_align_crop_96x112_for_idcard1m.py \
        --image-list=/workspace/failed_image_list.txt \
        ...
        --save-dir=/disk2/data/FACE/face-idcard-1M/face-idcard-1M-mtcnn-aligned-96x112-failed \
        --gpu-id=0 \
        --nsplits=1 \
        --split-id=0 > ./process-log-failed.txt &
else
    ...
    for gpu_id in `seq 0 $last_gpu_id`; do
        ...
        for i in `seq $start $end`; do
            ...
            nohup python batch_mtcnn_align_crop_96x112_for_idcard1m.py \
                ...
                --gpu-id=$gpu_id \
                --nsplits=$splits \
                --split-id=$i   > '/workspace/process-log-failed-'$splits'-'$i'.txt' &
