...
#    if not SAVE_INTO_SEPERATE_JSON:
    ...
    t1 = time.clock()
    detector = MtcnnDetector(model_path)
    t2 = time.clock()
    ...
    for line in fp:
        ...
        _minsize = int(min(img.shape[0:2]) * MINSIZE_RATIO)
        _minsize = max(minsize, _minsize)

        bboxes, points = detector.detect_face(img, _minsize,
        ...
        if SAVE_INTO_SEPERATE_JSON:
            fn_json = osp.join(save_dir, osp.splitext(
                osp.basename(img_path))[0] + '.json')
